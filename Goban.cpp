...
Goban::Goban(int xBoard, int yBoard)
{
  _xBoard = xBoard;
  _yBoard = yBoard;
  std::vector<int> box = std::vector<int> (xBoard, 0);
  _board = std::vector<std::vector<int> > (yBoard, box);
}
...
int Goban::getXBoard() const
{
  return _xBoard;
}

int Goban::getYBoard() const
{
  return _yBoard;
}

std::vector<std::vector<int> >  Goban::getBoard() const
{
  return _board;
}
